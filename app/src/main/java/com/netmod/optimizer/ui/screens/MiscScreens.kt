...
import com.netmod.optimizer.ui.components.LabeledRow
...
fun SettingsScreen(vm: AppViewModel) {
    ...
    LazyColumn(Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        item {
            SectionCard {
                CardTitle("About connection on Android")
                Text("This port uses the system VpnService instead of Wintun. xray-core runs through the libXray bridge; drop the AAR into app/libs to enable live tunnelling.",
...
fun AboutScreen(vm: AppViewModel) {
    ...
    LazyColumn(Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item {
            SectionCard {
                Text("NetMod Optimizer", color = p.fg1, fontSize = 22.sp, fontWeight = FontWeight.Bold)
                Text("VLESS Edge Optimizer — Android Edition v4", color = p.fg3, fontSize = 13.sp, modifier = Modifier.padding(top = 4.dp))
                Text("Ported from the single-file desktop app. All modules preserved: scanner, xray config, BPB/CF worker deploy, QR, profiles. Connection adapted to Android's VpnService + xray-core (libXray).",
                    color = p.fg2, fontSize = 13.sp, modifier = Modifier.padding(top = 10.dp))
            }
        }
        item {
            SectionCard {
                CardTitle("Developer log", "Internal activity / errors — tap Diagnostic Report for a full system snapshot")
                Button(
                    onClick = { vm.shareDiagnostics() },
                    modifier = Modifier.fillMaxWidth().padding(vertical = 8.dp),
                    colors = ButtonDefaults.buttonColors(containerColor = p.blue),
                ) { Text("🩺 Diagnostic Report") }
                vm.logLines.take(60).forEach { Text(it, color = p.fg3, fontSize = 11.sp) }
                if (vm.logLines.isEmpty()) Text("—", color = p.fg3)
            }
        }
    }
}
